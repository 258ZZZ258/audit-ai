...
| audit-biz 边界 `/v1/query` | 2026-07-02;local fix | 按 audit-biz boundary v1.1.0 回灌:新增无状态/无身份 `POST /v1/query` + `X-Internal-Token` + SSE 五事件 + 轻量 citation(score) + request_id trace;Retriever 加 contextvar scope 把 Java 预计算 filters 下推到 Milvus 前置过滤。`audit_project` 的 project_id/owner 因本仓 schema 未接入先 422 拒绝,避免静默放宽 | query_devlog audit-biz Track B |
