...
工作树)。细节见 `docs/query-agent-docs/query_devlog.md` §5.4 节。**集成 3 passed + 全 query 模型门 223 passed / 2 skipped(干净栈 + 真 BGE-M3),零回归。**
...
- **门控**:集成 `test_sparse_boost_integration` **3 passed**(名次升/扩展召回/双关等价,干净栈+真 BGE-M3);
  全 query 模型门 **223 passed / 2 skipped** 无回归 + `test_pg_io` 新种子 inert;ruff 全绿。RTM:§5.4 → ✅,计数 47✅/35🟡/33❌。
