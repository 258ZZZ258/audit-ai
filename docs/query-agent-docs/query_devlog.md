...
## 知识库真实字段收口:法律法规 / 公司制度库 / 行业案例(2026-07-03)

> 背景:业务侧确认真实知识库字段与 B-API 原型四 Tab 字段不完全一致;两个对比模块本轮暂不接入。

- **收口策略**:按业务要求直接清理结果项旧字段,`RegulationHit`、`RegulatoryRuleHit`、`CaseHit`
  的 JSON 输出只保留真实知识库字段;不再输出旧的 `title/doc_no/match_score/clause_excerpt`
  等兼容字段,也不再输出 `display_fields` 包装层。`clauses` 属查询条款命中,不是本轮知识库模块字段,
  暂不改。
...
- **缺口诚实处理**:`file_number`、`compliance_review_record` 当前无权威来源,字段值为空时省略,
  不臆造;标签来自业务类别、适用对象、案例类型等已落库字段,不把密级 `perm_tag` 外显为业务标签。
