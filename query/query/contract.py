...
class RegulationHit:
    ...
    字段对齐真实知识库模块;日期为 ISO 串(装配层 ``date.isoformat()``);可选缺失省略。
    """
    ...
    def to_dict(self) -> dict:
        return _compact(
            seq=self.seq, file_name=self.file_name, document_number=self.document_number,
            issuing_department=self.issuing_department, effective_date=self.effective_date,
            validity_level=self.validity_level, validity_status=self.validity_status,
            business_category=self.business_category or None,
...
class RegulatoryRuleHit:
    """法律法规命中(旧名监管规则,外规,SPEC-API §4.3)。字段对齐真实知识库模块。"""
    ...
    def to_dict(self) -> dict:
        return _compact(
            seq=self.seq, file_name=self.file_name, document_number=self.document_number,
            issuing_unit=self.issuing_unit, issue_date=self.issue_date,
...
class CaseHit:
    ...
    def to_dict(self) -> dict:
        return _compact(
            case_name=self.case_name, document_number=self.document_number,
            issuing_unit=self.issuing_unit, issue_date=self.issue_date, case_type=self.case_type,
